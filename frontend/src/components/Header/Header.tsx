...
export default function Header() {
  ...
  return (
    ...
    >
      ...
      <div className={`${isMenuOpen ? "flex" : "hidden"} md:hidden flex-col absolute top-0 left-0 w-full h-screen bg-[#1f1f1f] items-center justify-center gap-8`}>
        <div onClick={() => { changeScreen("home"); setIsMenuOpen(false); }}>
          <Text color={currentScreen === "home" ? "#FFFFFF" : "#A8AFC1"} size="1rem">Início</Text>
        </div>
        <div onClick={() => { changeScreen("about"); setIsMenuOpen(false); }}>
          <Text color={currentScreen === "about" ? "#FFFFFF" : "#A8AFC1"} size="1rem">Sobre</Text>
        </div>
        <div onClick={() => { changeScreen("projects"); setIsMenuOpen(false); }}>
          <Text color={currentScreen === "projects" ? "#FFFFFF" : "#A8AFC1"} size="1rem">Projetos</Text>
        </div>
        <div onClick={() => { changeScreen("partners"); setIsMenuOpen(false); }}>
          <Text color={currentScreen === "partners" ? "#FFFFFF" : "#A8AFC1"} size="1rem">Parceiros</Text>
        </div>
        <div onClick={() => { changeScreen("contact"); setIsMenuOpen(false); }}>
      ...
      <section className="hidden md:flex gap-[3dvw]">
